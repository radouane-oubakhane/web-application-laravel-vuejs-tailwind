...
let addRappot = ref(false);
const delRap = ref(false);
const delId = ref(null);
const updateRap = ref(false);
...
const search = ref('');
const tab = ref(props.rapports);
const message = ref(true);
const errors = ref(true);

watch(search, () => {
    tab.value = props.rapports.filter(
        rapport => rapport['intitule'].toUpperCase().includes(search.value.toUpperCase())
    )
})
...
const submit = () => {
    addRappot.value =! addRappot.value
    Inertia.post('/rapports/store', form)
}
function updateRapport(id) {
    updateRap.value = !updateRap.value
    rapport.id = id
}
function update() {
    Inertia.post('/rapports/edit', rapport)

}
function delRapport(rapport) {
    delId.value = rapport
    delRap.value = !delRap.value
}
...
function download(rapport) {
    downl.id = rapport
    Inertia.get('/rapport/download', downl)
}
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container mx-auto px-6 py-8">
                        <h3 class="text-gray-700 text-3xl font-medium">Rapports</h3>
                        <div class="px-6 py-8">
                            <button @click="addRappot =! addRappot" type="button" class="text-white bg-gradient-to-br from-green-600 to-green-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Ajouter une rapport </button>
                        </div>
                        ...
                        <div v-if="rapports.length" class="flex flex-col mt-8">
                            <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                                <div
                                    ...
                                    <table class="min-w-full">
                                        ...
                                        <tr v-for="rapport in rapports" v-if="!annuler">
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 text-gray-900">{{rapport.intitule}}</div>
                                            </td>

                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 text-gray-900">{{rapport.detail}}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <p v-if="rapport.etat"
                                            ...
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <button
                                                    ...
                                                    @click="updateRapport(rapport.id)">
                                                    Editer
                                                </button>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <button
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800"
                                                    @click="delRapport(rapport.id)">
                                                    Supprimer
                                                </button>
                                            </td>

                                        </tr>
                                        <tr v-for="rapport in tab" v-if="annuler">
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 text-gray-900">{{rapport.intitule}}</div>
                                            </td>
    ...
    <div v-if="addRappot"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Ajouter un nouveau rapport</h3>
                        <form @submit.prevent="submit"
                            ...
                            <div class="mb-2">
                                <p class="block mb-2 text-sm font-bold text-gray-700">
                                    Fichier
                                    <span class="text-xs pt-1 italic text-red-500"> fichier PDF</span>
                                </p>
                                <div class="w-full px-3 py-2 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">

                                    <label for="fichier">cliquez ici pour télécharger votre fichier PDF</label>
                                </div>
                                <input
                                    @input="form.fichier = $event.target.files[0]"
                                    class="cursor-pointer absolute block opacity-0 pin-r pin-t"
                                    id="fichier"
                                    name="fichier"
                                    type="file"
    ...
    <div v-if="delRap"
        ...
        <div class="w-full  max-w-lg p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white ">
            <div class="">
                <div class="text-center p-5 flex-auto justify-center">
                    <h2 class="text-xl font-bold py-4 ">Êtes-vous sûr?</h2>
                    <p class="text-sm text-gray-500 px-8">Voulez-vous vraiment supprimer cet rapport?</p>
                </div>
                <div class="p-3  mt-2 text-center space-x-4 md:block">
                    <button @click="delRap = !delRap" class="mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-md hover:shadow-lg hover:bg-gray-100">
                        Non
                    </button>
                    <Link
    ...
    <div v-if="updateRap"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Ajouter Un Rapport</h3>
                        <form @submit.prevent="submit"
                              class="px-8 pt-6 pb-2 mb-4 bg-white rounded">
                            <div class="mb-2">
                            ...
                            <div class="mb-2">
                                ...
                                <input
                                    @input="rapport.fichier = $event.target.files[0]"
                                    class="cursor-pointer absolute block opacity-0 pin-r pin-t"
                                    id="fichier"
                                    name="fichier"
                                    type="file"
