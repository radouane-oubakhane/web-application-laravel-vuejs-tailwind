...
let addPartenaire = ref(false);
const delPart = ref(false);
const delId = ref(null);
const updatePart = ref(false);
...
const search = ref('');
const tab = ref(props.partenaires);
const message = ref(true);
const errors = ref(true);

watch(search, () => {
    tab.value = props.partenaires.filter(
        partenaire => partenaire['nom'].toUpperCase().includes(search.value.toUpperCase())
    )
})
...
const partenaire = useForm({
    id: null,
    nom: null,
    detail: null,
    photo: null
});
function updateParteanire(id) {
    updatePart.value = !updatePart.value
    partenaire.id = id
}
function update() {
    Inertia.post('/partenaires/edit', partenaire)

}
function delPartenaire(partenaire) {
    delId.value = partenaire
    delPart.value = !delPart.value
}
const submit = () => {
    addPartenaire.value = !addPartenaire.value
    Inertia.post('/partenaires/store', form)
}

</script>

<template>
    <Head title="Partenaire" />
    <div>
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container mx-auto px-6 py-8">
                        <h3 class="text-gray-700 text-3xl font-medium">Partenaires</h3>
                        <div class="px-6 py-8">
                            <button @click="addPartenaire = !addPartenaire" type="button" class="text-white bg-gradient-to-br from-green-600 to-green-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Ajouter un Partenaire</button>
                        </div>
                        ...
                        <div v-if="partenaires.length !== 0" class="flex flex-col mt-8">
                            <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
                                <div
                                    ...
                                    <table class="min-w-full">
                                        ...
                                        <tr v-if="annuler"
                                            v-for="partenaire in tab">
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="flex items-center">
                                                    <div class="ml-4">
                                                        <div class="text-sm leading-5 font-medium text-gray-900">{{partenaire.nom}}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 text-gray-900">{{partenaire.detail}}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <button
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800"
                                                    @click="updateParteanire(partenaire.id)">
                                                    Editer
                                                </button>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <button
                                                    ...
                                                    @click="delPartenaire(partenaire.id)">
                                                    Supprimer
                                                </button>
                                            </td>
                                        </tr>
    ...
    <div v-if="addPartenaire"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Ajouter un nouveau Partenaire</h3>
                        <form @submit.prevent="submit"
                            ...
                            <div class="mb-6 text-center">
                                <button
                                    class="w-full px-4 py-2 font-bold text-white bg-lab-red rounded-full hover:bg-red-700 focus:outline-none focus:shadow-outline"
                                    @click="addPartenaire =!addPartenaire"
                                >
                                    Annuler
                                </button>
                            </div>
    ...
    <div v-if="delPart"
        ...
        <div class="w-full  max-w-lg p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white ">
            <div class="">
                <div class="text-center p-5 flex-auto justify-center">
                    <h2 class="text-xl font-bold py-4 ">Êtes-vous sûr?</h2>
                    <p class="text-sm text-gray-500 px-8">Voulez-vous vraiment supprimer cet partenaire?</p>
                </div>
                <div class="p-3  mt-2 text-center space-x-4 md:block">
                    <button @click="delPart = !delPart" class="mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-md hover:shadow-lg hover:bg-gray-100">
                        Non
                    </button>
                    <Link
                        ...
                        @click="delPart = !delPart"
                        :href="`/partenaires/${delId}`"
                        method="delete"
                        as="button"
                        type="button">
                        Oui
    ...
    <div v-if="updatePart"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Editer un partenaire</h3>
                        <form
                            class="px-8 pt-6 pb-2 mb-4 bg-white rounded">
                            <div class="mb-2">
                                <label class="block mb-2 text-sm font-bold text-gray-700" for="nom">
