...
let addEquipe = ref(false);
const delEq = ref(false);
const delId = ref(null);
const updateEq = ref(false);
...
const search = ref('');
const tab = ref(props.equipes);
const message = ref(true);
const errors = ref(true);

watch(search, () => {
    tab.value = props.equipes.filter(
        equipe => equipe['nom'].toUpperCase().includes(search.value.toUpperCase())
    )
})
...
function updateEquipe(id) {
    updateEq.value = !updateEq.value
    equipe.id = id
}
function update() {
    Inertia.post('/equipes/edit', equipe)

}
function delEquipe(equipe) {
    delId.value = equipe
    delEq.value = !delEq.value
}
const submit = () => {
    addEquipe.value =! addEquipe.value
    Inertia.post('/equipes/store', form)
}
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container mx-auto px-6 py-8">
                        <h3 class="text-gray-700 text-3xl font-medium">Equipes</h3>
                        <div class="px-6 py-8">
                            <button @click="addEquipe =!addEquipe" type="button" class="text-white bg-gradient-to-br from-green-600 to-green-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2">Ajouter une équipe</button>
                        </div>
                        ...
                        <div v-if="equipes.length !== 0" class="flex flex-col mt-8">
                                    ...
                                    <table class="min-w-full">
                                        ...
                                        <tr v-if="!annuler"
                                            ...
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="flex items-center">
                                                    <div class="ml-4">
                                                        <div class="text-sm leading-5 font-medium text-gray-900">{{equipe.nom}}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td
                                                ...
                                                {{equipe.chefEquipe}}</td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <div class="text-sm leading-5 text-gray-900">{{equipe.detail.slice(0, 100)}}...</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                                <button
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-orange-100 text-orange-800"
                                                    @click="updateEquipe(equipe.id)">
                                                    Editer
                                                </button>
                                            </td>
                                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
    ...
    <div v-if="addEquipe"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Ajouter une nouvelle équipe</h3>
                        <form @submit.prevent="submit"
                            ...
                            <div class="mb-4">
                                ...
                                <select class="w-full px-3 py-2 mb-3 text-sm bg-white leading-tight text-gray-700 text-center border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                                        name="fonction"
                                        v-model="form.chefEquipe">
                                    <option v-for="chercheur in chercheurs" :value="chercheur.id" :key="chercheur.id">{{chercheur.nom}} {{chercheur.prenom}}</option>
                                </select>
                            </div>
                            <div class="mb-4">
    ...
    <div v-if="delEq"
        ...
        <div class="w-full  max-w-lg p-5 relative mx-auto my-auto rounded-xl shadow-lg  bg-white ">
            <div class="">
                <div class="text-center p-5 flex-auto justify-center">
                    <h2 class="text-xl font-bold py-4 ">Êtes-vous sûr?</h2>
                    <p class="text-sm text-gray-500 px-8">Voulez-vous vraiment supprimer cette équipe?</p>
                </div>
                <div class="p-3  mt-2 text-center space-x-4 md:block">
                    <button @click="delEq = !delEq" class="mb-2 md:mb-0 bg-white px-5 py-2 text-sm shadow-sm font-medium tracking-wider border text-gray-600 rounded-md hover:shadow-lg hover:bg-gray-100">
                        Non
                    </button>
                    <Link
    ...
    <div v-if="updateEq"
        ...
        <div class="container mx-auto">
            <div class="flex justify-center px-6 my-12">
                <div class="w-full xl:w-3/4 lg:w-11/12 flex justify-center z-10 shadow-md">
                    <div class="w-full lg:w-7/12 bg-white p-5 rounded-lg">
                        <h3 class="pt-4 text-2xl text-center text-lab-purple">Editer Une équipe</h3>
                        <form
                              class="px-8 pt-6 pb-2 mb-4 bg-white rounded">
                            <div class="mb-2">
                                <label class="block mb-2 text-sm font-bold text-gray-700" for="nom">
